...
class TEDARegressor:
  def __init__(self,nI,nR,N1,N2,N3,tau,m,eol=0.1,n_rls=3,fator=None,ep=0.1,wta=False):
    nO = nI
    self.g = 1
    self.gCreated = 1
    ...
    self.df =1
    self.dt=1
    self.rls = RLS(n_rls,0.9999,1000)
  ...
  def denormalize(self,Max,Min):
      self.HI = (self.HI*(Max-Min))+Min
      self.HIp = (self.HIp*(Max-Min))+Min
      self.HIpL = (self.HIpL*(Max-Min))+Min
      self.HIpU = (self.HIpU*(Max-Min))+Min

  def denormalize2(self,Max,Min):
      self.HI = (self.HI*(Max-Min))+Max
      self.HIp = (self.HIp*(Max-Min))+Max
      self.HIpL = (self.HIpL*(Max-Min))+Max
      self.HIpU = (self.HIpU*(Max-Min))+Max

  def uncertainty_TS(self,yP,yR):
      eP = abs(self.rls.predict(yP))
      eR = abs(yR-yP)
      self.rls.adapt(yP,eR)
      self.HIp = np.append(self.HIp,yP)
      self.HIpL = np.append(self.HIpL,yP-eP*self.ep)
      self.HIpU = np.append(self.HIpU,yP+eP*self.ep)
  ...
  def MAPE2(self,epsilon=1e-10):
    y_true, y_pred = self.HI.copy(), self.HIp.copy()
    dif = np.abs(y_true-y_pred)
    return np.mean(dif)
  
  def NDEI(self):
    ndei=np.array([0])
    for i in range(2,1+len(self.HI)):
      rmse=np.sqrt(mean_squared_error(self.HI[:i], self.HIp[:i]))
      #print(rmse)
      std = (np.std(self.HIp[:i]))
