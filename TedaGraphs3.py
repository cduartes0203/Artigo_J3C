...
def plot_multiple_DSI(teda_list, w=6, h=4, lnwdth=0.75, ftcks=7, flbl=8.5, fttl=8.0, 
    ...
    fig.legend(handles, labels, loc='lower center', ncol=len(labels), fontsize=flgnd, framealpha=0.85,columnspacing=0.7)
    plt.suptitle('Granular DSI', fontsize=10)
    plt.tight_layout(rect=rect)
    plt.subplots_adjust(wspace=0.05)
...
def plot_multiple_HIandDSI(teda_list, w=cm(14), h=cm(10), lnwdth=0.75, ftcks=7,
                            flbl=8, fttl=8, flgnd=7, m1=2, m2=5, m3=1.2,
                              bearings=None, png=True, out=None, 
                              rect=[-0.025, 0.06, 1.02, 1.1], title='Granular DSI and HI'):
    n = len(teda_list)
    ...
    for idx, teda in enumerate(teda_list):
        ...
        if idx > 0:
            ax.set_ylabel('')
            ax.tick_params(labelleft=False,labelsize=5,axis='y',length=0.1,)
        ax.set_title(f'{bearings[idx]}', fontsize=fttl)
        ax.grid(False)
    ...
    if out is not None:
        plt.savefig(out + 'Bearings_HiDsi' + ext, dpi=500, transparent=False)
...
def plot_multiple_RMSE(teda_list, w=6, h=2, lnwdth=0.75,
    ...
    for idx, teda in enumerate(teda_list):
        ...
        ax.set_ylim(0, 130)
        ax.set_xlabel("Cycle", fontsize=flbl)
        ax.set_ylabel("RMSE/Cycle", fontsize=flbl)
        ax.set_ylim(0, 140)
        if idx > 0:
            ax.set_ylabel('')
            ax.tick_params(labelleft=False,labelsize=5,axis='y',length=0.1,)
    ...
    fig.legend(handles, labels, loc='lower center', ncol=len(labels), fontsize=flgnd, framealpha=0.85,columnspacing=0.7)
    plt.suptitle('RUL Prediction RMSE', fontsize=10)
    plt.tight_layout(rect=rect)
    plt.subplots_adjust(wspace=0.05)

    if out is not None:
        plt.savefig(out + 'Bearings_RMSE' + ext, dpi=500, transparent=False)
...
def plot_RUL_ax(ax,teda, startX=None, startY=None, endX=None, endY=None,
                ...
                lw1=1.25, lw2=1, mk1=5, mk2=1.5, ftcks=7, flbl=8.5, fttl=8, flgnd=7,
                dotsize=2, loc=None, ncol=None, tol=25,
    ...
    # Linhas principais
    ax.plot(x[:end], rulR[:end], linestyle='-', linewidth=lw1*0.8, color='black', label="R-RUL")
    #ax.plot(x[:end], rulP[:end], linestyle='-', linewidth=lw1*0.6, color='blue', label="P-RUL")
    ax.plot(x[:end], rulU[:end], linestyle='--', linewidth=lw1, color='blue')
    ax.plot(x[:end], rulL[:end], linestyle='--', linewidth=lw1, color='blue', label='Uncertainty')
    ax.fill_between(x[:end], (1 - tol / 100) * rulR[:end], (1 + tol / 100) * rulR[:end],
                    color='lightgray', alpha=0.25, label=f"T-{tol}%", linewidth=lw1)
    ax.plot([teda.eolX, teda.eolX], [-1, np.max(teda.rulU) * 1.2], color='black', linestyle=':', linewidth=lw2)
    ax.plot([-200, 200], [eolRUL, eolRUL], color='black', linestyle=':', linewidth=lw2)
    ...
    if (teda.rulR[0]) >=100: yticks = [i * 25 for i in range(1,20)]
    if (teda.rulR[0]) <50: yticks = [i * 10 for i in range(1,20)]
    ...
    ax.set_xlim(startX, endX)
    ax.set_ylim(startY, endY)
    if xlabel: ax.set_xlabel("Cycle", fontsize=flbl)
    if ylabel: ax.set_ylabel("RUL/Cycle", fontsize=flbl)
...
def plot_RUL_GRID(teda,startX=None,name='Bearings',title='Granular RUL Prediction' ,bearings=None,
                                  w=cm(14), h=cm(12), out=None, png=True,
                                    rect=[-0.025,0.06,1.025,1.02]):
    fig, axes = plt.subplots(2, 2, figsize=(w, h), gridspec_kw={'width_ratios': [5, 5]})

    if bearings==None: bearings=['Bearing 1','Bearing 2','Bearing 3','Bearing 4']
    
    handles1, labels1 = plot_RUL_ax(axes[0,0], teda[0], startX=26, endY=90,
                                  name='a) '+bearings[0],xlabel=False, title=False)
    handles2, labels2 = plot_RUL_ax(axes[0,1], teda[1], startX=48, endY=148,
                                  name='b) '+bearings[1],xlabel=False,ylabel=False, title=False)
    handles3, labels3 = plot_RUL_ax(axes[1,0], teda[2], startX=32, endY=113,
                                  name='c) '+bearings[2], title=False)
    handles4, labels4 = plot_RUL_ax(axes[1,1], teda[3], startX=8, endX=40, endY=35,
                                  name='d) '+bearings[3],ylabel=False, title=False)
...
def plot_RUL_LIST(teda,startX=None,name='Bearings2',title='Granular RUL Prediction' ,bearings=None,
                                  w=cm(14), h=cm(18), out=None, png=True,rect=[-0.025,0.035,1.02,1.02]):
    
    fig, axes = plt.subplots(5, 1, figsize=(w, h), gridspec_kw={'width_ratios': [5]})

    if bearings==None: bearings=['Bearing 1','Bearing 2','Bearing 3','Bearing 4','Bearing 5']
    handles1, labels1 = plot_RUL_ax(axes[0], teda[0],name='a) '+bearings[0],
                                  startX=20, startY=-2, endY=90,title=False,xlabel=False)
    handles2, labels2 = plot_RUL_ax(axes[1], teda[1],name='b) '+bearings[1],
                                  startX=48, startY=-9, endY=148,title=False,xlabel=False)
    handles3, labels3 = plot_RUL_ax(axes[2], teda[2],name='c) '+bearings[2],
                                  startX=29.3, startY=-2, endY=113,title=False,xlabel=False)
    handles4, labels4 = plot_RUL_ax(axes[3], teda[3],name='d) '+bearings[3],
                                  startX=35, startY=-5, endY=95, title=False,xlabel=False)
    handles5, labels5 = plot_RUL_ax(axes[4], teda[4],name='e) '+bearings[4],
                                  startX=12.5, startY=0, endX=39, endY=31, title=False)
    handles = [handles1,handles2,handles3,handles4,handles5]
